...
/**
 * Linear publishes nothing a machine can read, and saying so is a positive
...
const service: HealthCheckDefinition = {
  key: "service",
  title: "Linear platform status",
  kind: "service",
  covers: ["*"],
  severity: "informational",
  unavailable: {
    reason:
      "status.linear.app is a human page with no JSON API or feed. The derived `auth:*` credential check is the only automatable liveness signal.",
  },
};
