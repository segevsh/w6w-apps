...
import action from "../../actions/like-delete.ts";

const RECORD_URI = "at://did:plc:me/app.bsky.feed.like/3old";

/** Given the record's own URI, it deletes directly. */
Deno.test("like-delete: deletes the record when given its own URI", async () => {
 ...
 */
Deno.test("like-delete: given a post, it finds the caller's own record first", async () => {
  const found = ok({ posts: [{ uri: POST_URI, cid: "c", viewer: { like: RECORD_URI } }] });
...
Deno.test("like-delete: nothing to remove is not an error", async () => {
...
Deno.test("like-delete: someone else's record is refused", async () => {
  const { ctx, calls } = mockCtx([], { display });
  await assertRejects(
    async () => await action.execute!({ uri: "at://did:plc:other/app.bsky.feed.like/3old" }, ctx),
    Error,
...
Deno.test("like-delete: needs something to act on", async () => {
  const { ctx } = mockCtx([], { display });
  await assertRejects(async () => await action.execute!({}, ctx), Error);
});

Deno.test("like-delete: is idempotent, since removing twice is the same end state", () => {
