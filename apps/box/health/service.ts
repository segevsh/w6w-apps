/**
 * Is Box up? — Atlassian Statuspage.
 *
 ...
 *   - `network.allow` — status.box.com is deliberately NOT on the app's egress
 ...
 * https://status.box.com/api/v2/summary.json (Atlassian Statuspage — same
 * shape as status.dropbox.com's).
 */
...
const STATUS_HOST = "status.box.com";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Box platform status",
  description:
    "Atlassian Statuspage rollup for status.box.com, with per-component detail. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
