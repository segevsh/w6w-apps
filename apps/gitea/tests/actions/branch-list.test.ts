...
Deno.test("branch-list: reads the bare-array collection", async () => {
  const { ctx, calls } = mockCtx([{ status: 200, body: [{ name: "main" }] }], conn);
  assertEquals(await action.execute!({ repo: "web" }, ctx), [{ name: "main" }]);
  assertEquals(new URL(calls[0].url).pathname, "/api/v1/repos/acme/web/branches");
});

Deno.test("branch-list: returnAll walks pages until a short one", async () => {
...
Deno.test("branch-list: a blank repository fails before any request", async () => {
