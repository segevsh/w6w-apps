...
Deno.test("pipeline-get: GETs one record and unwraps the data array", async () => {
...
Deno.test("pipeline-get: sends no `fields` query parameter when none was asked for", async () => {
  const { ctx, calls } = mockBiginCtx([{ body: { data: [{ id: "42" }] } }]);
  await action.execute({ recordId: "42" }, ctx);
  assertEquals(new URL(calls[0].url).search, "");
});

Deno.test("pipeline-get: throws when Bigin answers 200 with no record", async () => {
