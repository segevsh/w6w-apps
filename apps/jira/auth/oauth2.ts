 ...
 * gateway at `api.atlassian.com/ex/jira/{cloudId}`, and the cloud id has to be
 * resolved after the token is issued — `afterConnect` calls
 * `/oauth/token/accessible-resources` for it and records it on the connection,
 * which is what `lib/client.ts` reads.
 *
...
const oauth2: AuthDefinition = {
  ...
  async afterConnect(_input, ctx) {
    ...
    const me = await ctx.fetch(
      `https://api.atlassian.com/ex/jira/${first.id}/rest/api/3/myself`,
    );
    const user = me.ok
      ? await me.json().catch(() => ({})) as {
        accountId?: string;
        displayName?: string;
        emailAddress?: string;
      }
    ...
    return {
      ...
      siteName: first.name,
      siteUrl: first.url,
      user: { id: user.accountId, displayName: user.displayName, email: user.emailAddress },
    };
  },
};
