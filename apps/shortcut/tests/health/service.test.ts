...
/** Trimmed from the live response measured 2026-09-15 (7,848 bytes, 23 components). */
function summary(overrides: Record<string, unknown> = {}) {
  return {
    ...
    components: [
      { id: "r4lqrtxqhv0y", name: "API", status: "operational", group: false },
      { id: "kcgq9fj1tw9y", name: "Web App", status: "operational", group: false },
      { id: "5526vdxn4f5s", name: "Integrations", status: "operational", group: true },
      {
        id: "t0bvf10dh1cv",
        name: "Zendesk",
        status: "operational",
        group: false,
        group_id: "5526vdxn4f5s",
      },
    ],
...
Deno.test("service: probes the status host, not the API host", () => {
  assertEquals(STATUS_URL, "https://status.shortcut.com/api/v2/summary.json");
  assertEquals(service.network?.allow, ["status.shortcut.com"]);
...
Deno.test("service: group containers are excluded from the component report", async () => {
  ...
  assertEquals("5526vdxn4f5s" in (report.components ?? {}), false);
});
...
Deno.test("service: components are keyed by vendor id", async () => {
  ...
  assertEquals(report.components?.r4lqrtxqhv0y?.message, "API");
});
...
Deno.test("service: an incident is reported with the affected components named", async () => {
  ...
  assertEquals(report.components?.r4lqrtxqhv0y?.state, "down");
...
Deno.test("service: a page that stops self-identifying as Shortcut's reports unknown", async () => {
  const { ctx } = mockCtx([
    {
