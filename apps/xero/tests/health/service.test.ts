...
Deno.test("service: maps the Statuspage indicator and per-component detail", async () => {
  const { ctx, calls } = mockCtx([{
    body: {
      ...
      components: [
        { name: "Invoicing, Bills & Quotes", status: "operational" },
        { name: "Xero Platform and Settings", status: "operational" },
  ...
  assertEquals(out.components?.["invoicing-bills-quotes"], { state: "ok" });
  assertEquals(out.components?.["a-group-header"], undefined);
  assertEquals(calls[0].url, "https://status.xero.com/api/v2/summary.json");
});
