/**
 * Is CircleCI up? — Atlassian Statuspage, the same platform Netlify,
 * Cloudflare and SendGrid publish on.
 *
 ...
 *   - `network.allow` — status.circleci.com is deliberately NOT on the app's
 ...
 *
 * Verified live 2026-08-01: https://status.circleci.com/api/v2/summary.json
 * (page metadata reads "CircleCI" served from the Statuspage v2 API shape).
 */
import type { HealthCheckDefinition, HealthState } from "@w6w/types";

/** Statuspage's rollup indicators. */
...
const STATUS_HOST = "status.circleci.com";

const service: HealthCheckDefinition = {
  key: "service",
  title: "CircleCI platform status",
  description:
    "Atlassian Statuspage rollup for status.circleci.com, with per-component detail. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
