...
Deno.test("service: reads the Statuspage summary.json on www.harveststatus.com", async () => {
  const { ctx, calls } = mockCtx([{ body: { status: { indicator: "none" }, components: [] } }]);
  const report = await service.check!({}, ctx);
  assertEquals(calls[0].url, "https://www.harveststatus.com/api/v2/summary.json");
