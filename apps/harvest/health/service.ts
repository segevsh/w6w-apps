/**
 * Is Harvest up? — Atlassian Statuspage at harveststatus.com.
 *
 ...
 *     Running it per Connection would multiply one useful call by the number
 *     of users and is a good way to get rate-limited by a status page.
 *   - `credential: "none"` (also the default) — no Connection is supplied
 *     and `sign` never runs, so this reports even before anyone has
 *     connected.
 *   - `network.allow` — www.harveststatus.com is deliberately NOT on the
 ...
 * carries the per-component breakdown — one probe reporting many things,
 * which is the point of a report over a boolean.
 */
...
const STATUS_HOST = "www.harveststatus.com";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Harvest platform status",
  description:
    "Atlassian Statuspage rollup for www.harveststatus.com, with per-component detail. " +
  ...
  async check(_input, ctx) {
    ...
    // nothing about Harvest, and reporting that as an outage would be a lie.
