/**
 * Snowflake's status page (status.snowflake.com) is Statuspage.io-powered and
 ...
 * structured field, so — same as the Mistral example in the App-building
 * guide — we read the newest update per incident (`latest`, not `entries`)
 * and treat anything not opening with "Resolved" as still open.
 */
import type { HealthCheckDefinition } from "@w6w/types";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Snowflake platform status",
  description: "Reads status.snowflake.com's Atom history feed for open (non-Resolved) incidents.",
  kind: "service",
  covers: ["*"],
  feed: { url: "https://status.snowflake.com/history.atom" },
