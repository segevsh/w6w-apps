...
Deno.test("index: the icon is the vendor's mark, on the pack's canvas", async () => {
  ...
  assert(
    svg.includes("M 125.346344 63.5 C 125.346344"),
    "the vendor's geometry changed — the mark was redrawn",
  );
  // The "Generated by Pixelmator Pro" line this used to assert sat *outside*
  // the root element, and the re-frame keeps only what the root contains. It
  // was provenance, not artwork; the path above is the provenance that counts.
});
