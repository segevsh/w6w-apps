 ...
 * the sibling `linkedin` (member/social) app probes.
 *
 * Verified live 2026-08-15: `https://www.linkedin-apistatus.com/api/v2/summary.json`
 * answers `200 application/json`, page name "LinkedIn API", url
 * `https://www.linkedin-apistatus.com` — LinkedIn's official developer API
 * status page (distinct from the generic consumer-site `linkedin-status.com`
 * tracker), covering the platform as a whole rather than a Marketing-API-only
 * feed. LinkedIn publishes no separate status page scoped to the Advertising
 * API alone, so this is the best available signal for "is LinkedIn's API
 * infrastructure up" — the same reasoning the sibling app used.
 *
 * Note the live response currently carries an **empty `components` array** —
 * `status.indicator` (page-level) is the only populated field at the moment
 * this was checked. The component-mapping logic below still has to handle
 * that (and any future non-empty list) correctly rather than assuming shape
 * from one observation.
 *
 * Annotation:
 *   - `kind: "service"`, `scope: "app"` (default) — one shared result, not
 *     one per Connection; this app declares two auth methods, and running a
 *     status-page fetch per Connection would multiply one call needlessly.
 *   - `credential: "none"` (default) — reports even before anyone has
 *     connected either auth method.
 ...
 *     business calling it.
 *   - `severity` left at the `degraded` default: an incident here is evidence
 *     about every Connection, but LinkedIn's Advertising API being briefly
...
const service: HealthCheckDefinition = {
  ...
  // status host must never see either auth method's token.
  credential: "none",
