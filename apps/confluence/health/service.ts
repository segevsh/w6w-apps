/**
 * Is Confluence up? — Atlassian Statuspage.
 *
 ...
 *   - `network.allow` — confluence.status.atlassian.com is deliberately NOT on the app's
 ...
 *
 * Atlassian runs one Statuspage per product; `status.atlassian.com` is the
 * cross-product rollup. This app is Confluence, so it probes the Confluence
 * page — a Jira incident is not a Confluence incident, and the `jira` app in
 * this pack probes its own.
 *
 * Verified 2026-08-18: `confluence.status.atlassian.com/api/v2/summary.json`
 * returns a real Statuspage payload (page id `4g2my7tbhjsq`, name
 * "Confluence", 5,490 bytes) and `status.json` a distinct 230-byte one — two
 * real endpoints, not one catch-all document answering every path.
 */
...
const STATUS_HOST = "confluence.status.atlassian.com";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Confluence platform status",
  description:
    "Atlassian Statuspage rollup for confluence.status.atlassian.com, with per-component detail. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
