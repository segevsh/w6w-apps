import type { ActionDefinition } from "@w6w/types";
import { GhostClient } from "../lib/client.ts";

interface Input {
  filter?: string;
  order?: string;
  limit?: number;
  page?: number;
  includeTags?: boolean;
  includeAuthors?: boolean;
}

interface Output {
  items: unknown[];
  meta?: unknown;
}

const listPosts: ActionDefinition<Input, Output> = {
  key: "list-posts",
  type: "read",
  resource: "post",
  title: "List Posts",
  description: "List posts on a single page. Set `page` to walk further pages.",
  params: [
    {
      key: "filter",
      label: "Filter",
      type: "string",
      hint: "Ghost NQL filter, e.g. `status:published` or `tag:news+status:draft`.",
    },
    { key: "order", label: "Order", type: "string", default: "published_at desc" },
    { key: "limit", label: "Limit", type: "number", default: 15 },
    { key: "page", label: "Page", type: "number", default: 1 },
    { key: "includeTags", label: "Include Tags", type: "boolean", default: false },
    { key: "includeAuthors", label: "Include Authors", type: "boolean", default: false },
  ],
  output: [
    { key: "items", type: "array", label: "Posts" },
    { key: "meta", type: "object", label: "Pagination metadata" },
  ],

  execute(input, ctx) {
    const client = GhostClient.fromConnection(ctx);
    const include = [input.includeTags && "tags", input.includeAuthors && "authors"]
      .filter(Boolean)
      .join(",");
    return client.browse("posts", {
      filter: input.filter,
      order: input.order,
      limit: input.limit ?? 15,
      page: input.page ?? 1,
      include: include || undefined,
    });
  },
};

export default listPosts;
