  ...
  const out = await service.check!(
    {
      feed: {
        entries: [],
        latest: [entry("1", "API degraded", "Resolved - fixed")],
        fetchedAt: "now",
      },
    },
    mockCtx().ctx,
...
Deno.test("service: degraded when a latest entry is not Resolved", async () => {
  const out = await service.check!(
    {
      feed: {
        entries: [],
        latest: [entry("1", "Docs loading slowly", "Investigating - looking into it")],
        fetchedAt: "now",
      },
    },
    mockCtx().ctx,
  ...
  assertEquals(out.message, "Docs loading slowly");
});
