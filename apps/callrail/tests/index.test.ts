...
/**
 * The four "does something happen in the world exactly once" perform actions:
 * an outbound call is placed, a company/tag/form-submission is created, or a
 * text is sent. Retrying a dropped connection here risks doing the thing
 * twice.
 */
Deno.test("index: real side-effect performs (create, send, call) are not idempotent", () => {
  for (
    const key of [
      "call-create",
      "company-create",
...
/** The converse: these converge on the same end state no matter how many times they run. */
Deno.test("index: update/delete performs are marked idempotent", () => {
  for (
    const key of [
      "call-update",
      "company-update",
 ...
 * the very assertion it's explaining.
 */
...
Deno.test("index: no action reads a credential — signing is the auth hook's job", async () => {
  for (const a of app.actions) {
    ...
    assert(!/\btoken\s*=\s*["'`]/i.test(src), `${a.key}: builds a raw token string`);
...
Deno.test("index: no action hard-codes a host or an absolute URL", async () => {
  for (const a of app.actions) {
    const src = await actionSource(a.key);
    assert(!/callrail\.com/.test(src), `${a.key}: contains a CallRail host literal`);
...
Deno.test("index: every action that isn't the account-discovery one requires an accountId param", () => {
  for (const a of app.actions) {
    if (a.key === "account-list") continue;
    const keys = (a.params ?? []).map((p) => p.key);
    assert(keys.includes("accountId"), `${a.key}: missing accountId param`);
  }
});

// --- auth --------------------------------------------------------------------

Deno.test("index: the auth probe is /a.json, never an account-scoped path", async () => {
  const src = code(await Deno.readTextFile(new URL("../auth/api-token.ts", import.meta.url)));
  assert(src.includes('"/a.json"'), "auth probe no longer hits /a.json");
});
...
Deno.test("index: the manifest allows the API host and not the status host", async () => {
  const manifest = JSON.parse(
    ...
    w6w: {
      ...
      appearance: { icon: { url: string; sizes?: Record<string, string> } };
      categories: string[];
    };
  };
  assertEquals(manifest.w6w.id, "io.w6w.callrail");
  assert(manifest.w6w.network.allow.includes("api.callrail.com"));
  // The status host belongs to the health check's own allowlist, not the app's.
  assert(!manifest.w6w.network.allow.includes("status.callrail.com"));
...
Deno.test("index: the icon file exists and is byte-identical to what was placed for this app", async () => {
  const bytes = await Deno.readFile(new URL("../assets/icon.png", import.meta.url));
  // A PNG signature, and non-trivial size — this is a real image, not a stub.
  assertEquals([...bytes.slice(0, 8)], [137, 80, 78, 71, 13, 10, 26, 10]);
  assert(bytes.length > 100, "icon.png is suspiciously small");
});
