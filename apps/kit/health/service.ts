/**
 * Is Kit up? — Atlassian Statuspage.
 *
 ...
 *   - `network.allow` — status.kit.com is deliberately NOT on the app's egress
 ...
 * the per-component breakdown — and Kit's components are worth having, since
 * `API` and `Email sending` are separately reported from `Application` and the
 * `Marketing Site`. A workflow can be entirely fine while the marketing site is
 * down, and entirely broken while it is up.
 *
 * Verified live 2026-08-03: status.kit.com serves the Statuspage v2 API with
 * components MCP, Integrations, Marketing Site, Application, Email sending, API.
 * The legacy status.convertkit.com and convertkit.statuspage.io resolve to the
 * same page; status.kit.com is the current canonical host.
 */
...
const STATUS_HOST = "status.kit.com";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Kit platform status",
  description:
