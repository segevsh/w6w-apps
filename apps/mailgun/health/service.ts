/**
 * Is Mailgun up? — Atlassian Statuspage.
 *
 ...
 *     every Connection regardless of region, so the host runs it once and
 *     shares the result.
 ...
 *   - `network.allow` — status.mailgun.com is deliberately NOT on the app's
 ...
 *
 * `summary.json` rather than `status.json`: same single request, but it also
 * carries the per-component breakdown — Mailgun's page lists 11 components
 * (API, SMTP, Outbound Delivery, Inbound, Events & Logs, Stats, Email
 * Validation, Inbox Placement, Spam Trap Network, Email Previews, Control
 * Panel), and one call reports all of them at once. Verified live 2026-07-31.
 */
...
const STATUS_HOST = "status.mailgun.com";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Mailgun platform status",
  description:
    "Atlassian Statuspage rollup for status.mailgun.com, with per-component detail. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
