...
Deno.test("index: the comment stripper actually strips, so the guards above mean something", () => {
  // Guards the guard: if `code()` silently stopped stripping, the two sandbox
  // checks would still pass and would still be worthless.
...
Deno.test("index: the quota check declares no extra egress, because it is signed", () => {
