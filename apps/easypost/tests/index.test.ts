...
/** Anything that creates a second object, or spends a second time. */
...
Deno.test("index: the manifest names only the API host", () => {
  assertEquals(manifest.w6w.network.allow, ["api.easypost.com"]);
  assertEquals(manifest.w6w.id, "io.w6w.easypost");
});

/**
 * The distinction the whole app rests on: quoting and spending are separate
 * steps, and `service` in a creation request would collapse them.
 */
...
Deno.test("index: buying offers a price ceiling", () => {
  const buy = app.actions.find((a) => a.key === "shipment-buy")!;
...
/**
 * An address is somebody's home and a name is a person. A run log records ids,
 * counts and outcomes.
 */
