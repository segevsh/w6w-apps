/**
 * Is Google Slides up? — the Google Workspace Status Dashboard.
 *
 ...
 *     never hard-fails a target on its own. The signal is already **narrowed**
 *     to this product (see the `service_name` filter), so the default stands
 *     rather than being downgraded to informational: what it reports is Google
 *     Slides, not a Workspace-wide rollup covering surfaces this app never
 *     touches.
 *
 * "Google Slides" is a first-class product on the dashboard (confirmed against
 * `products.json`, which lists it with id `DvWBgkXVhodA3WYwgZoB`), so the filter
 * below matches a real `service_name` rather than silently matching nothing.
 *
 ...
 * running. The feed covers all of Workspace, so it is filtered to Google Slides —
 * a Meet outage is not a Google Slides outage.
 */
import type { HealthCheckDefinition, HealthState } from "@w6w/types";
import { worstHealthState } from "@w6w/types";

const STATUS_HOST = "www.google.com";
const SERVICE_NAME = "Google Slides";
...
const service: HealthCheckDefinition = {
  key: "service",
  title: "Google Slides platform status",
  description:
