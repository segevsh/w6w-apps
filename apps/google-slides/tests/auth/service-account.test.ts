...
Deno.test("service-account: exchange posts a JWT-bearer assertion scoped to presentations", async () => {
  ...
  // Same minimal grant as the oauth2 method — no Drive, no Sheets.
  assertEquals(claims.scope, "https://www.googleapis.com/auth/presentations");
  assertEquals(claims.sub, undefined);
});

Deno.test("service-account: an explicit `scopes` override is honoured", async () => {
  const { ctx, calls } = mockCtx([{ body: { access_token: "t" } }]);
  await exchangeForAccessToken({
    email: "sa@p.iam.gserviceaccount.com",
    privateKey: await generatePem(),
    scopes:
      "https://www.googleapis.com/auth/presentations https://www.googleapis.com/auth/spreadsheets",
  }, ctx.fetch);
...
Deno.test("service-account: a 200 without access_token is still an error", async () => {
  // The 2xx-that-means-failure case on the credential path.
  ...
  // The message must not carry the signed assertion.
  assertEquals(/eyJ/.test(err.message), false);
});
