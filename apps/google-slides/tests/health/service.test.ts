...
Deno.test("service: probes the Workspace status dashboard, not an API host", () => {
  ...
  // No credential is supplied and `sign` never runs, so this reports before
  // anyone has connected. Widening the allowlist is only permitted because of
  // that unsigned posture.
  assertEquals(service.credential, undefined);
});

Deno.test("service: reports ok when no Google Slides incident is open", async () => {
...
Deno.test("service: an open Slides outage reports down with the vendor description", async () => {
  const { ctx } = mockCtx([{
    body: [{
      service_name: "Google Slides",
      status_impact: "SERVICE_OUTAGE",
      external_desc: "Users cannot open presentations",
    }],
  }]);
  const report = await service.check!({}, ctx);
  assertEquals(report.state, "down");
  assertEquals(report.message, "Users cannot open presentations");
});

Deno.test("service: a disruption is degraded, information is ok", async () => {
  const { ctx } = mockCtx([{
    body: [
      { service_name: "Google Slides", status_impact: "SERVICE_DISRUPTION", external_desc: "Slow" },
      { service_name: "Google Slides", status_impact: "SERVICE_INFORMATION", external_desc: "FYI" },
    ],
  }]);
  const report = await service.check!({}, ctx);
...
Deno.test("service: an unrelated Workspace outage does not drag Slides down", async () => {
  const { ctx } = mockCtx([{
    body: [{ service_name: "Google Meet", status_impact: "SERVICE_OUTAGE", external_desc: "Down" }],
  }]);
