...
Deno.test("index: every action that writes cells says so at the form", () => {
  // The one thing a user can get wrong that fails silently: keying cells by
  // column title. Every write action must state the rule on its own param.
  for (const key of ["add-rows", "update-rows"]) {
    const a = app.actions.find((x) => x.key === key)!;
    const cells = (a.params ?? []).find((p) => p.key === "cells")!;
    assert(/COLUMN ID/.test(cells.hint ?? ""), `${key}: cells hint does not state the id rule`);
  }
});
...
Deno.test("index: the quota check is a declared absence, with no hook and no extra egress", () => {
