/**
 * Is Intercom up? — Atlassian Statuspage.
 *
 ...
 *
 * The host is `www.finstatus.com`, not `status.intercom.com`. Intercom moved its
 * status page onto Fin's (Intercom's AI product) Statuspage: `status.intercom.com`
 * and `www.intercomstatus.com` both 301/302-redirect to `www.finstatus.com`,
 * which serves the real Atlassian Statuspage JSON. We probe the final host
 * directly so no cross-host redirect has to be followed inside the worker.
 *
 ...
 * `down`; the roll-up caps it at `degraded` anyway (severity defaults to
 * `degraded` for kind `service`), so the distinction is purely what an operator
 * sees.
 */
...
const STATUS_HOST = "www.finstatus.com";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Intercom platform status",
  description:
    "Atlassian Statuspage rollup for Intercom (served at www.finstatus.com), with per-component detail. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
