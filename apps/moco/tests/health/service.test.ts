  ...
  const out = await service.check!(
    {
      feed: {
        entries: [],
        latest: [entry("1", "Limited Availability", "Resolved - All systems are now operational.")],
        fetchedAt: "now",
      },
    },
    mockCtx().ctx,
...
Deno.test("service: degraded when a latest entry is not Resolved", async () => {
  const out = await service.check!(
    {
      feed: {
        entries: [],
        latest: [entry("1", "Limited Availability", "Investigating - looking into it")],
        fetchedAt: "now",
      },
    },
    mockCtx().ctx,
  ...
  assertEquals(out.message, "Limited Availability");
});
