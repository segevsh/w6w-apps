/**
 * Is Google Ads up? — the **Google Ads Status Dashboard**, not the Workspace one.
 *
 * This is the one place this app deliberately diverges from its google-*
 * siblings. Gmail, Calendar, Drive, Tasks and friends all probe
 * `www.google.com/appsstatus/dashboard/incidents.json`, and reusing that here
 * would look consistent and be wrong: Google Ads is not a Workspace product and
 * does not appear in that feed. Verified rather than assumed, in both
 * directions —
 *
 *   - `https://ads.google.com/status/publisher/products.json` lists 16 products
 *     and **"Google Ads API" is one of them**, alongside "Google Ads" itself
 *     (also AdMob, AdSense, Google Ad Manager, Display & Video 360, Search Ads
 *     360, …). So an API-specific incident does surface here, under its own
 *     `service_name`, rather than being folded into the advertiser product.
 *   - the incident feed lives beside it at
 *     `https://ads.google.com/status/publisher/incidents.json` and uses the
 *     same schema as the Workspace dashboard's — `begin`/`end`,
 *     `status_impact`, `external_desc`, plus `service_name`,
 *     `affected_products[]` and `most_recent_update`.
 *
 * Two feed quirks that would otherwise produce wrong verdicts, both observed in
 * live data and handled below:
 *
 *   - `service_name` values arrive with a **leading space** (`" Google Ads"`),
 *     so matching is trimmed and case-insensitive.
 ...
 *   - `network.allow` — `ads.google.com` is not the API host this app talks to
 *     (that is `googleads.googleapis.com`) and is deliberately absent from the
 *     app's own egress allowlist; an action has no business reaching it.
 *     Widening it for this one hook is permitted precisely because the posture
 *     is unsigned.
 *   - `severity` defaults to `degraded` for this kind, so a Google Ads incident
...
const SERVICES = ["Google Ads API", "Google Ads"];
...
const service: HealthCheckDefinition = {
  key: "service",
  title: "Google Ads platform status",
  description:
    "Open incidents for Google Ads and the Google Ads API on the Google Ads Status Dashboard. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
