/**
 * Is Spotify up? — Atlassian Statuspage.
 *
 ...
 *   - `network.allow` — spotify.statuspage.io is deliberately NOT on the app's
 ...
 * point of a report over a boolean. Confirmed live at
 * https://spotify.statuspage.io/api/v2/summary.json (Atlassian Statuspage,
 * checked 2026-08-01) — top-level `status`, `components`, `incidents`,
 * `scheduled_maintenances`, same shape as GitHub's and Box's status pages.
 *
 * Spotify also publishes a separate, narrower status page at
 * status.spotify.dev for the Web API specifically. spotify.statuspage.io is
 * used here because it is the one confirmed to expose the machine-readable
 * `/api/v2/summary.json` Statuspage contract this hook depends on.
 */
...
const STATUS_HOST = "spotify.statuspage.io";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Spotify platform status",
  description:
    "Atlassian Statuspage rollup for spotify.statuspage.io, with per-component detail. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
