/**
 * Is THIS connection's site reachable, and is the Gravity Forms REST API
 * actually registered on it?
 *
 ...
 *     availability is a property of their infrastructure. `health/service.ts`
 ...
 * discovery document. It is chosen over any `gf/v2` route because every one of
 * those is capability-gated, so an authenticated probe would conflate "the site
 * is down" with "this credential lacks a capability". The discovery document
 * separates three distinct failures:
 *
 ...
 *   3. 200 but `namespaces` has no `gf/v2` -> WordPress is fine, but the
 *      Gravity Forms REST API v2 is switched off at Forms -> Settings -> REST API
 *
 * (3) is the interesting one: it is the most common setup mistake for this App
 * and is completely invisible to a plain reachability check.
 *
...
const site: HealthCheckDefinition = {
  ...
  description:
    "Unauthenticated `GET /wp-json/` against this connection's site — proves the host resolves, " +
    "the WordPress REST API is enabled, and the `gf/v2` namespace is registered.",
  kind: "dependency",
  scope: "connection",
  credential: "context",
  covers: ["*"],
  ...
  async check(_input, ctx) {
    ...
    if (Array.isArray(namespaces) && !namespaces.includes(REST_NAMESPACE)) {
      return {
        state: "degraded",
        message:
          `site is up but the \`${REST_NAMESPACE}\` namespace is not registered — enable the ` +
          "Gravity Forms REST API v2 at Forms -> Settings -> REST API",
        ttlSeconds: 120,
      };
    }
