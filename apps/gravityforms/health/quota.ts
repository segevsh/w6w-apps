 ...
 *
 * Gravity Forms' REST API v2 rides on the site's own WordPress REST API. Neither
 * documents a rate limit nor returns usage headers a probe could read — the only
 * ceiling is whatever the customer's web host, PHP configuration or a security
 * plugin imposes, and none of those publish a number.
 *
...
const quota: HealthCheckDefinition = {
  ...
  unavailable: {
    reason:
      "Gravity Forms REST API v2 rides on the site's own WordPress REST API. Neither documents a " +
