...
Deno.test("basic: test probes <site>/wp-json/gf/v2/forms with the credential", async () => {
  const { ctx, calls } = mockCtx([{ status: 200, body: { "1": { title: "Contact" } } }]);
  const result = await auth.test({ credential: cred }, ctx);
  assertEquals(result.ok, true);
  assertEquals(calls[0].url, "https://example.com/wp-json/gf/v2/forms");
...
Deno.test("basic: test honours a subdirectory install", async () => {
  ...
  assertEquals(calls[0].url, "https://site.com/blog/wp-json/gf/v2/forms");
});
...
Deno.test("basic: test explains a 404 as the REST API being switched off", async () => {
  const { ctx } = mockCtx([{ status: 404, body: "" }]);
  const result = await auth.test({ credential: cred }, ctx);
  assertEquals(result.ok, false);
  assert((result.message ?? "").includes("Forms -> Settings -> REST API"));
});
...
Deno.test("basic: test falls back to the status when the error body is not JSON", async () => {
...
Deno.test("afterConnect: publishes a normalised siteUrl, host and username — no credential", async () => {
  ...
  const display = await auth.afterConnect!(
    { credential: { ...cred, siteUrl: "https://site.com/blog/wp-json/gf/v2/" } },
    ctx,
