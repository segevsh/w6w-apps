/**
 * Is Google Tasks up? — the Google Workspace Status Dashboard.
 *
 ...
 *     (that is tasks.googleapis.com), and an action has no business reaching it.
 ...
 *
 * Coverage was verified rather than assumed: Google Tasks is a first-class
 * product on the dashboard — it appears in
 * `https://www.google.com/appsstatus/dashboard/products.json` under the title
 * "Google Tasks" — so an incident affecting it does surface in this feed under
 * that `service_name`.
 *
 ...
 * The feed covers all of Workspace, so it is filtered to Google Tasks — a Meet
 * outage is not a Google Tasks outage.
 */
import type { HealthCheckDefinition, HealthState } from "@w6w/types";
import { worstHealthState } from "@w6w/types";

const STATUS_HOST = "www.google.com";
const SERVICE_NAME = "Google Tasks";
...
const service: HealthCheckDefinition = {
  key: "service",
  title: "Google Tasks platform status",
  description:
    "Open incidents for Google Tasks on the Google Workspace Status Dashboard. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
