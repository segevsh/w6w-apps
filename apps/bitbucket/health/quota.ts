/**
 * How much headroom is left on THIS credential — Bitbucket.
 *
 * Annotation:
 *
 ...
 *
 * Probe: `GET /2.0/user`, the same scope-free whoami the auth `test` hook uses.
 * Bitbucket has no headroom endpoint; the counters ride on every response, so
 * the cheapest call that any credential can make is the probe.
 */
...
/** Bitbucket's reset header is epoch SECONDS. */
  ...
  async check(_input, ctx) {
    ...
    return {
      ...
      quota: [{
        ...
        limit,
        remaining,
        resetAt: isoFromEpoch(res.headers.get("x-ratelimit-reset")),
        unit: "requests",
      }],
      ttlSeconds: 60,
    };
