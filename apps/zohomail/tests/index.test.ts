...
/**
 * Creating a folder/label or sending an email starts a new resource on every
 * call — retrying a dropped response would create a duplicate, not converge
 * on the same state.
 */
Deno.test("index: creates and sends are not marked idempotent", () => {
  for (const key of ["folder-create", "label-create", "message-send"]) {
    assertEquals(app.actions.find((a) => a.key === key)?.idempotent, false, key);
  }
});

/**
 * The converse: these four converge on the same end state no matter how many
 * times they run, so the runtime is free to retry them.
 */
Deno.test("index: read/unread, move, label-apply and delete are marked idempotent", () => {
  for (
    const key of ["message-mark-read", "message-move", "message-label-apply", "message-delete"]
  ) {
 ...
 * else. An action that hard-coded a `mail.zoho.<tld>` host — or accepted one
 * as a param — could be pointed somewhere the manifest never allowlisted.
 */
/**
 * URLs inside `hint:`/`description:`/`placeholder:` prose are documentation
 * (e.g. a link to Zoho's own search-syntax page), not requests — excluded the
 * same way `_tools/audit.ts`'s real egress scan excludes them, so this check
 * catches a hard-coded fetch target without also flaming a doc link.
 */
...
Deno.test("index: no action hard-codes a host or an absolute URL", async () => {
  for (const a of app.actions) {
    ...
    assert(
      !/zoho\.com|zoho\.eu|zoho\.in|zohocloud/.test(src),
...
Deno.test("index: the manifest allows every region's API host and none of the accounts hosts", async () => {
  ...
  assertEquals(manifest.w6w.id, "io.w6w.zohomail");
  for (const region of REGIONS) {
