...
const action: ActionDefinition<Input> = {
  ...
  params: [
    chatId,
    {
      ...
      type: "string",
      required: true,
      hint: "HTTP(S) URL, or a `file_id` of a video already on Telegram's servers.",
    },
    caption,
    parseMode,
    { key: "duration", label: "Duration (seconds)", type: "number" },
    { key: "supportsStreaming", label: "Supports streaming", type: "boolean" },
    deliveryOptions,
  ],
  output: messageOutput,

  execute(input, ctx) {
    return new TelegramClient(ctx).call("sendVideo", {
      body: {
