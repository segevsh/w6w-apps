/**
 * Is this connection's Freshdesk domain reachable?
 *
 * Annotation, and why each axis is what it is:
 *
 *   - `kind: "dependency"` — Freshdesk publishes no machine-readable status
 ...
 *   - No `network.allow` is declared: `*.freshdesk.com` is already on the
 ...
 * failure (the hook throws), a 404 (domain gone) or a 5xx counts as down.
 */
...
const domain: HealthCheckDefinition = {
  ...
  description:
    "Unauthenticated request to this connection's Freshdesk domain. A 401 passes — it proves the account is serving; credential validity is the `auth:*` check's job.",
  kind: "dependency",
  scope: "connection",
  credential: "context",
  covers: ["*"],
