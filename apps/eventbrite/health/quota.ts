/**
 * How much headroom is left on THIS credential — Eventbrite.
 *
 * Annotation:
 *
 ...
 *
 * Probe: `GET /v3/users/me/`, the same whoami the auth `test` hook uses.
 * Eventbrite has no headroom endpoint; the allowance is per token per hour and
 * rides on the response as `X-Rate-Limit-*`. It is the app's only automatable
 * signal beyond credential liveness — Eventbrite's status page is human-only,
 * which is why `service` is declared `unavailable`.
 */
...
const quota: HealthCheckDefinition = {
  ...
  description: "Hourly allowance remaining on this token, read off the `X-Rate-Limit-*` headers.",
  kind: "quota",
  covers: ["*"],
  ...
  async check(_input, ctx) {
    ...
    if (remaining === undefined) {
      // Eventbrite does not document the headers as guaranteed, so absence is
      // "we cannot tell", not "no quota left".
      return { state: "unknown", message: "response carried no X-Rate-Limit-* headers" };
    }

    return {
      state: headroom(remaining, limit),
      quota: [{ id: "hourly", limit, remaining, unit: "requests" }],
      ttlSeconds: 60,
    };
  },
};
