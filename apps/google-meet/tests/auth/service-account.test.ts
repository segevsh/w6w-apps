...
Deno.test("buildAssertion: encodes header and payload as base64url JWT with Meet scopes", () => {
  ...
  assertEquals(payloadJson.scope, MEET_SCOPES);
});
...
Deno.test("service-account: test issues the same Meet probe as OAuth", async () => {
  const { ctx, calls } = mockCtx([{ status: 200, body: { conferenceRecords: [] } }]);
  const result = await auth.test({ credential: { accessToken: "tok" } }, ctx);
  assertEquals(result.ok, true);
  assertEquals(new URL(calls[0].url).pathname, "/v2/conferenceRecords");
  assertEquals(calls[0].headers["authorization"], "Bearer tok");
});

Deno.test("service-account: afterConnect echoes the service account identity", async () => {
  const { ctx } = mockCtx();
  const out = await auth.afterConnect!(
    { credential: { clientEmail: "sa@p.iam.gserviceaccount.com" } },
    ctx,
  ) as Record<string, unknown>;
  assertEquals(out.clientEmail, "sa@p.iam.gserviceaccount.com");
});
