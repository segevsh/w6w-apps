...
Deno.test("app: declares the single basic auth method", () => {
...
Deno.test("app: no send action is marked idempotent — a retry would deliver twice", () => {
  for (const action of app.actions) {
    if (!action.key.startsWith("send-")) continue;
