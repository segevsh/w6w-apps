/**
 * Is Google Chat up? — the Google Workspace Status Dashboard.
 *
 ...
 *   - `network.allow` — www.google.com is not the API host this app talks to
 *     (that is chat.googleapis.com), and an action has no business reaching it.
 *     Widening the allowlist for this one hook is permitted precisely because
 ...
 *
 * Coverage was verified rather than assumed: Google Chat is a first-class
 * product on the dashboard — it appears in
 * `https://www.google.com/appsstatus/dashboard/products.json` under the title
 * "Google Chat" — so an incident affecting it does surface in this feed under
 * that `service_name`. (The retired "Classic Hangouts" is a separate product
 * entry and is deliberately not matched.)
 *
 * Google publishes an incident FEED rather than a current-state rollup, so "up"
 * is the absence of an open incident: an entry with no `end` is still running.
 * The feed covers all of Workspace, so it is filtered to Google Chat — a Meet
 * outage is not a Google Chat outage.
 */
import type { HealthCheckDefinition, HealthState } from "@w6w/types";
import { worstHealthState } from "@w6w/types";

const STATUS_HOST = "www.google.com";
const SERVICE_NAME = "Google Chat";
...
const service: HealthCheckDefinition = {
  key: "service",
  title: "Google Chat platform status",
  description:
    "Open incidents for Google Chat on the Google Workspace Status Dashboard. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
