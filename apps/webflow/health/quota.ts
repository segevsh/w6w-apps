/**
 * How much headroom is left on THIS credential — Webflow.
 *
 * Annotation:
 *
 ...
 *
 * Probe: `GET /sites`, the same scope-free call the auth `test` hooks use.
 * Webflow has no dedicated headroom endpoint; the counters ride on every
 * response as `X-RateLimit-Limit` / `X-RateLimit-Remaining`, a single bucket
 * metered per minute (60 rpm on Starter/Basic, 120 rpm on paid plans). The
 * header names are documented and contractual, so their absence is reported as
...
const quota: HealthCheckDefinition = {
  ...
  description:
    "Per-minute request allowance remaining, read off the `X-RateLimit-*` response headers.",
  kind: "quota",
  covers: ["*"],
  ...
  async check(_input, ctx) {
    ...
    const bucket: HealthQuota = { id: "requests", limit, remaining, unit: "requests" };
