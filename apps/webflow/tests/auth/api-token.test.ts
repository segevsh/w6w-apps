...
Deno.test("api-token: sign appends Bearer using credential.apiToken", async () => {
  ...
  const out = await auth.sign!({ request, credential: { apiToken: "wf-abc" } }, ctx);
  assertEquals(out.headers["authorization"], "Bearer wf-abc");
});

Deno.test("api-token: test hits /v2/sites and reports ok", async () => {
  const { ctx, calls } = mockCtx([{ status: 200, body: { sites: [] } }]);
  const result = await auth.test({ credential: { apiToken: "wf-abc" } }, ctx);
  assertEquals(result.ok, true);
  const url = new URL(calls[0].url);
  assertEquals(url.origin, "https://api.webflow.com");
  assertEquals(url.pathname, "/v2/sites");
  assertEquals(calls[0].headers["authorization"], "Bearer wf-abc");
});
