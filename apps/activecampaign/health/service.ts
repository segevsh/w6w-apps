/**
 * Is ActiveCampaign up? — Atlassian Statuspage.
 *
 ...
 *     check), "is there quota left" (`quota`), or "is THIS account's API host
 *     reachable" (`site` — a per-account host, not the vendor's platform).
 ...
 *   - `network.allow` — status.activecampaign.com is deliberately NOT on the
 *     app's egress allowlist (which is `"*"` only because the per-account API
 *     host is genuinely arbitrary); a status host still gets its own
 *     unsigned, hook-scoped widening rather than blending into that.
 ...
 * the per-component breakdown (API, Email, SMS, CRM, Automations, Campaigns, …
 * across US/EU/APAC groups) — one probe reporting many things.
 */
...
const STATUS_HOST = "status.activecampaign.com";

const service: HealthCheckDefinition = {
  key: "service",
  title: "ActiveCampaign platform status",
  description:
    "Atlassian Statuspage rollup for status.activecampaign.com, with per-component, per-region detail. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
