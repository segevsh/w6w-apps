...
import action from "../../actions/check-group-list.ts";

Deno.test("check-group-list: reads the bare-array collection", async () => {
  const { ctx, calls } = mockCtx([{ status: 200, body: [{ id: "x1" }] }]);
  assertEquals(await action.execute!({}, ctx), [{ id: "x1" }]);
  assertEquals(new URL(calls[0].url).pathname, "/v1/check-groups");
});

Deno.test("check-group-list: returnAll walks pages until a short one", async () => {
