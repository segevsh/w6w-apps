/**
 * Is Acuity Scheduling up? — Atlassian Statuspage.
 *
 ...
 *   - `network.allow` — status.acuityscheduling.com is deliberately NOT on the
 ...
 *
 * `summary.json` rather than `status.json`: one request, and it carries both
 * the rollup indicator and the per-component breakdown — one probe reporting
 * many things. Verified live 2026-08-01: GET
 * https://status.acuityscheduling.com/api/v2/summary.json returns a
 * Statuspage-shaped body (`status.indicator`, `components[]`).
 */
...
const STATUS_HOST = "status.acuityscheduling.com";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Acuity Scheduling platform status",
  description:
    "Atlassian Statuspage rollup for status.acuityscheduling.com, with per-component detail. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
