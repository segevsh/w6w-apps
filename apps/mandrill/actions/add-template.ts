...
const addTemplate: ActionDefinition<Input> = {
  key: "add-template",
  type: "perform",
  resource: "template",
  title: "Add Template",
  description: "Create a new template (POST /templates/add.json).",
  idempotent: false,
  params: [
    ...
    { key: "publish", label: "Publish immediately", type: "boolean", default: true },
  ...
  execute(input, ctx) {
    ...
    const labels = input.labels
      ...
      : undefined;
    return client.request("/templates/add.json", {
      name: input.name,
