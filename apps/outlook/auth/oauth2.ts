 ...
 *     which is what "Outlook" means to a user. A deployment that must be
 *     restricted to one tenant registers its own app and overrides the URLs.
 *
 ...
 * `Mail.ReadWrite` (read, draft, update, move, delete), `Mail.Send` (send,
 * reply, forward), `Calendars.ReadWrite` (read + write events), `User.Read`
 * (the `test` / `afterConnect` probe), `offline_access` (refresh token).
 */
...
const oauth2: AuthDefinition = {
   ...
   * `User.Read`, so a credential that legitimately lacks a mail or calendar
   * scope still reports as live rather than as broken.
