...
Deno.test("quota: is declared absent — Graph exposes no headroom for Outlook", () => {
...
Deno.test("quota: the reason records the reactive signal and the published ceilings", () => {
  const reason = quota.unavailable!.reason;
  assert(reason.includes("429"));
  assert(reason.includes("Retry-After"));
  assert(reason.includes("10,000 requests per 10 minutes"));
});
