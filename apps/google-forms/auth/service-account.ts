 ...
 *
 * Forms-specific caveats — read these before choosing this method:
 *   - A service account is a Drive principal with no UI. A form it *creates*
 *     is owned by the service account and is not visible in any human's Drive;
 *     for anything an operator needs to open, create the form as a user or
 *     hand ownership over afterwards via Drive.
 *   - A form it *reads or edits* must be explicitly shared with the service
 *     account's email, exactly like sharing with a person. Google's sharing UI
 *     will show it under "Shared with…".
 *   - Domain-wide delegation (the `subject` field below) is the way to act as
 *     a real user in a Workspace domain, and is usually what you actually want
 *     for Forms.
...
interface ServiceAccountCredential {
  email: string;
  privateKey: string;
  /** Optional space-separated OAuth scopes. Defaults to forms + drive scopes. */
  scopes?: string;
...
const serviceAccount: AuthDefinition = {
  ...
  description:
    "Server-to-server auth using a Google service account. Paste the service account's email and PEM private key (from a downloaded JSON key). Share the target form with the service account's email, or use domain-wide delegation to impersonate a real user.",
  ...
  fields: [
    ...
    },
    {
      ...
      type: "string",
      hint:
        "Only for domain-wide delegation. The user email whose identity the service account should assume — usually what you want for Forms, so created forms land in a real person's Drive.",
    },
  ],
