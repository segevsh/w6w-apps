/**
 * Is Google Forms up? — the Google Workspace Status Dashboard.
 *
 ...
 *
 * "Google Forms" is a first-class product on the dashboard (confirmed against
 * `products.json`), so the filter below matches a real `service_name` rather
 ...
 * running. The feed covers all of Workspace, so it is filtered to Google Forms —
 * a Meet outage is not a Google Forms outage.
 */
import type { HealthCheckDefinition, HealthState } from "@w6w/types";
import { worstHealthState } from "@w6w/types";

const STATUS_HOST = "www.google.com";
const SERVICE_NAME = "Google Forms";
...
const service: HealthCheckDefinition = {
  key: "service",
  title: "Google Forms platform status",
  description:
    "Open incidents for Google Forms on the Google Workspace Status Dashboard. Unauthenticated and unsigned.",
  kind: "service",
  covers: ["*"],
