...
Deno.test("oauth2: requests the forms + drive scopes the API actually accepts", () => {
...
Deno.test("oauth2: afterConnect maps userinfo onto connection label data", async () => {
  const { ctx, calls } = mockCtx([
    { status: 200, body: { sub: "u1", name: "Ada", email: "ada@example.com" } },
  ]);
...
Deno.test("oauth2: afterConnect degrades to no label data when userinfo is denied", async () => {
