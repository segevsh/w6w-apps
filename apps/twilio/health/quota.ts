...
/**
 * Twilio exposes no headroom to read, so there is nothing to probe. Declared
...
const quota: HealthCheckDefinition = {
  ...
  unavailable: {
    reason:
      "Twilio publishes no headroom endpoint or rate-limit headers. Messaging queues rather than rejecting, and other resources apply concurrency limits, so there is no counter to read.",
  },
};
